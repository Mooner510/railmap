...
import {
  emptyRailFeatureCollection,
  RAIL_MAP_SOURCE_IDS,
  registerRailMapSourcesAndLayers,
  setRailMapSourceData,
} from "./map/railMapLayers";
import { bindRailMapInteractions } from "./map/railMapInteractions";
import {
...
function buildHighlightedRouteTransferFeature(
  stations: ValidRailMapStation[],
  stationIds: string[],
  branchIds: string[],
  branchIndex: Map<string, RailMapBranch>,
) {
  const stationIndex = new Map(stations.map((station) => [station.id, station]));
  const features: RailFeatureCollection["features"] = [];

  for (let index = 0; index < stationIds.length - 1; index += 1) {
    const branchId = branchIds[index];
    if (branchId && branchIndex.has(branchId)) continue;

    const fromStation = stationIndex.get(stationIds[index] ?? "");
    const toStation = stationIndex.get(stationIds[index + 1] ?? "");
    if (!fromStation || !toStation) continue;

    features.push({
      type: "Feature",
      properties: {
        id: `route-transfer-${features.length + 1}`,
      },
      geometry: {
        type: "LineString",
        coordinates: [
...
export default function RailMap({
  ...
  const routeBranchIndex = useMemo(
    () => new Map(branches.map((branch) => [branch.id, branch])),
    [branches],
  );
  ...
  const highlightedRouteTransferFeatures = useMemo(
    () =>
      buildHighlightedRouteTransferFeature(
        validStations,
        highlightedRouteStationIds,
        highlightedRouteBranchIds,
        routeBranchIndex,
      ),
    [highlightedRouteBranchIds, highlightedRouteStationIds, routeBranchIndex, validStations],
  );
  ...
  const highlightedRouteTransferFeaturesRef = useRef(highlightedRouteTransferFeatures);
  ...
  useEffect(() => {
    highlightedRouteTransferFeaturesRef.current = highlightedRouteTransferFeatures;
  }, [highlightedRouteTransferFeatures]);
    ...
    const initialize = () => {
      ...
      try {
        ...
        map.on("load", () => {
          ...
          registerRailMapSourcesAndLayers({
            map,
            branchFeatures: branchFeaturesRef.current,
            lineBranchFeatures: lineBranchFeaturesRef.current,
            routeFeatures: highlightedRouteFeaturesRef.current,
            routeTransferFeatures: highlightedRouteTransferFeaturesRef.current,
            transferAreaFeatures: transferGroupAreaFeaturesRef.current,
            transferIconFeatures: transferGroupIconFeaturesRef.current,
            stationFeatures: stationFeaturesRef.current,
            hasHighlightedRoute: highlightedRouteStationIds.length > 0,
          });

          bindRailMapInteractions({
            map,
            getBranchById: (branchId) => branchByIdRef.current.get(branchId),
            getStationById: (stationId) => stationByIdRef.current.get(stationId),
            getTransferGroupById: (groupId) =>
              transferGroupIndexRef.current.get(groupId),
            onSelectBranch: (branch) => onSelectBranchRef.current?.(branch),
            onSelectStation: (station) => onSelectStationRef.current?.(station),
            onSelectTransferGroup: (group) =>
              onSelectTransferGroupRef.current?.(group),
            onClearSelection: () => onClearStationRef.current?.(),
          });
  ...
  useEffect(() => {
    ...
    setRailMapSourceData(map, RAIL_MAP_SOURCE_IDS.branchLines, branchFeatures);
  ...
  useEffect(() => {
    ...
    setRailMapSourceData(map, RAIL_MAP_SOURCE_IDS.lineBranchLines, lineBranchFeatures);
  ...
  useEffect(() => {
    ...
    setRailMapSourceData(map, RAIL_MAP_SOURCE_IDS.routeLines, highlightedRouteFeatures);
  ...
  useEffect(() => {
    ...
    const empty = emptyRailFeatureCollection();
    setRailMapSourceData(
      map,
      RAIL_MAP_SOURCE_IDS.transferAreas,
      transferDetailVisible ? transferGroupAreaFeatures : empty,
    );
    setRailMapSourceData(
      map,
      RAIL_MAP_SOURCE_IDS.transferIcons,
      transferDetailVisible ? empty : transferGroupIconFeatures,
    );
  }, [
    transferDetailVisible,
    transferGroupAreaFeatures,
  ...
  useEffect(() => {
    ...
    setRailMapSourceData(map, RAIL_MAP_SOURCE_IDS.stations, stationFeatures);
